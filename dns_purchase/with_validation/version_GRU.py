...
config = {
    ...
    "training": {
        ...
        "learning_rate": 0.0007,  # Скорость обучения
        "sequence_length": 1,  # Длина последовательности
        "early_stopping_patience": 150,  # Ожидание улучшения ошибки
        "loss_target": 0.1,  # Целевая ошибка
        'train_test_split': 0.2
    },
    "forecast": {
        "forecast_weeks": 60,  # Количество недель в прогнозе
...
def forecast_future_weeks(df, model, scaler, sequence_length, forecast_weeks=60, freq='MS'):
    """
...
def run_pipeline(df,
                 ...
                 category=1, delivery=1, resp=1, freq='MS'):
    ...
    forecast_df = forecast_future_weeks(df, model, scaler, sequence_length, forecast_weeks, freq=freq)


    fig, axs = plt.subplots(4, figsize=(12, 12))
    fig.suptitle('Итерация 0')

    axs[0].plot(act_train, label="Actual Train", color="blue")
    axs[0].plot(preds_train, label="Predicted Train", color="red", linestyle="dashed")
    axs[0].legend()
    ...
    axs[2].plot(act_data, label='Actual Data', color="blue")
    axs[2].plot(preds_data, label='Predicted Data', color="red", linestyle="dashed")
    axs[2].legend()

    axs[3].plot(data['date'][-60:], data['product_count'][-60:], label='Actual Data', color='blue')
    axs[3].plot(forecast_df['Date'], forecast_df['Predicted'], color='orange', linestyle='dashed')
    axs[3].legend()
    fig.show()

    print(f'прогноз построен для категории {category}, канала поставки {delivery}, зоны ответственности {resp}')
