...
config = {
    "model": {
        ...
        "hidden_dim": 60  # Количество скрытых узлов
    },
    "training": {
...
def run_pipeline(df,
    ...
    batch_size = 8  # Пример размера батча
    sequence_length = config['training']['sequence_length']
    input_dim = config['model']['input_dim']

    # Создание фиктивного тензора для inputs
    ...
    axs[3].plot(data['date'][-60:], data['product_count'][-60:], label='Actual Data', color='#37B6CE')
